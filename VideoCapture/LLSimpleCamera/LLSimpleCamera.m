...
{
    return [self initWithQuality:AVCaptureSessionPresetPhoto position:LLCameraPositionRear videoEnabled:videoEnabled];
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super initWithCoder:aDecoder]) {
        [self setupWithQuality:AVCaptureSessionPresetPhoto
...
- (void)handlePinchGesture:(UIPinchGestureRecognizer *)recognizer
{
    ...
    if (allTouchesAreOnThePreviewLayer) {
        ...
        if ([_videoCaptureDevice lockForConfiguration:&error]) {
            ...
            NSLog(@"Magnication Factor = %f", _effectiveScale);
            NSLog(@"VOF = %f", _videoCaptureDevice.activeFormat.videoFieldOfView);
        }
        else {
            NSLog(@"%@", error);
        }
